...
module AHBVGA(
  ...
  reg [11:0] console_wdata;//data to write to console
  ...
  reg [11:0] r_text_color;
  reg [11:0] r_text_back_color; 
  ...
  vga_console 
    ...
    .text_rgb(console_rgb),//8->12
    .scroll(scroll),

    .cursor_x(r_cursor_x),
    .cursor_y(r_cursor_y),

    .text_color(r_text_color),//8->12
    .text_back_color(r_text_back_color),//8->12
  ...
  vga_image uvga_image(
    ...
    .image_data(image_wdata),//8->12
    .address(last_HADDR[16:2]),
    .image_rgb(image_rgb),//8->12
  ...
  begin
    if(!HRESETn)
      ...
      begin
        console_write <= 1'b1;
        console_wdata <= HWDATA[7:0];//input char ASCII
      end
    else
      begin
        console_write <= 1'b0;
  ...
  begin
    if(!HRESETn)
    begin
        r_text_color <= 12'h111;//8->12
        r_text_back_color <= 12'h000;//8->12
    end
    else if (last_HWRITE & last_HSEL & last_HTRANS[1] & HREADYOUT & sel_text_color )
    begin
      r_text_color <= HWDATA[11:0];//8->12
      r_text_back_color <= HWDATA[23:12];//8->12
    end
  end
